...
describe('log.marker', function()
  it('reads both marker syntaxes', function()
    assert.same('boom', log.marker('##[error]boom', 'error'))
    assert.same('boom', log.marker('::error::boom', 'error'))
    assert.same('careful', log.marker('::warning::careful', 'warning'))
  end)

  it('reads Forgejo markers carrying a location', function()
    assert.same('boom', log.marker('::error file=a.lua,line=3,col=1::boom', 'error'))
    assert.same('hm', log.marker('::warning file=a.lua,line=3::hm', 'warning'))
  end)

  it('ignores unrelated lines', function()
    assert.is_nil(log.marker('plain text', 'error'))
    assert.is_nil(log.marker('##[group]Setup', 'error'))
  end)
end)

describe('log.parse debug markers', function()
  it('colours the line but keeps the marker', function()
    local rows = log.parse(ts('01.0000000', '##[debug]Evaluating: format'), {})
    assert.same('CiDebug', rows[1].hl)
    assert.same(29, rows[1].conceal)
    assert.is_nil(rows[1].label)
    assert.is_truthy(rows[1].text:find('##%[debug%]'))
  end)

  it('reads the Forgejo spelling too', function()
